...
class TrafficLightGridNetwork(Network):
    ...
    * **speed_limit** : speed limit for all edges. This may be represented as a
    ...
    def get_path(self, start_edge, outer_edges, connections):
        paths = []
        stack = [[start_edge]]
        ...
        while len(stack) > 0:
            path = stack[-1]
            stack.pop()
            if path[-1] in outer_edges and len(path) > 1:
                if len(path) <= 6: 
                    paths.append(path)
                continue
            for edge in connections[path[-1]]:
                if edge not in path:
    ...
    def specify_routes(self, net_params):
        """See parent class."""
        self.specify_connections(net_params)
        self.get_outer_edges()
        outer_edges = set()
        
        for edge in self.outer_edges:
            outer_edges.add(edge['id'])
        connections = defaultdict(lambda : set())
        for key, value in self.connections_dict.items():
            for item in value:
                from_edge = item['from']
                to_edge = item['to']
                connections[from_edge].add(to_edge)

        paths = defaultdict(lambda : [])
        for edge in outer_edges:
            paths[edge] = self.get_path(edge, outer_edges, connections) 
         

        routes = defaultdict(list)
        for edge in outer_edges:
            all_paths = paths[edge]
            if len(all_paths) > 0:
                # print(len(all_paths))
                prob = 1.0 / len(all_paths)
                for path in all_paths:
                    temp = (path, prob)
                    routes[edge].append(temp)
        ...
        # [print(pair) for pair in routes.items()]
    ...
    def specify_connections(self, net_params):
        ...
        def new_con(side_from, side_to, from_id, to_id, lane, signal_group):
            return [{
                "from": side_from + from_id,
                "to": side_to + to_id,
                "fromLane": str(lane),
                "toLane": str(lane),
        ...
        for i in range(self.row_num):
            for j in range(self.col_num):
                ...
                for lane in range(self.vertical_lanes):
                    conn += new_con("bot","left",   node_id, "{}_{}".format(i, j),     lane, 1)
                    conn += new_con("bot","bot",    node_id, "{}_{}".format(i, j + 1), lane, 2)
                    conn += new_con("bot", "right", node_id, "{}_{}".format(i + 1, j), lane, 3)
                    
                    conn += new_con("top","right", right_node_id, "{}_{}".format(i + 1, j), lane, 1)
                    conn += new_con("top","top",   right_node_id, "{}_{}".format(i, j), lane, 2)
                    conn += new_con("top","left",  right_node_id, "{}_{}".format(i, j),     lane, 3)
    
    
                for lane in range(self.horizontal_lanes):
                    conn += new_con("right","bot",   node_id, "{}_{}".format(i, j + 1), lane, 1)
                    conn += new_con("right","right", node_id, "{}_{}".format(i + 1, j), lane, 2)
                    conn += new_con("right","top",   node_id, "{}_{}".format(i, j), lane,     3)
    
                    conn += new_con("left","top",  top_node_id, "{}_{}".format(i, j),     lane, 1)
                    conn += new_con("left","left", top_node_id, "{}_{}".format(i, j), lane, 2)
                    conn += new_con("left","bot",  top_node_id, "{}_{}".format(i, j + 1), lane, 3)
    
                node_id = "center{}".format(i * self.col_num + j)
                con_dict[node_id] = conn
        self.connections_dict = con_dict
        return con_dict
