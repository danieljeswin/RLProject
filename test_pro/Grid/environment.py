...
class TrafficLightGridEnv(Env):
    ...
    * discrete: determines whether the action space is meant to be discrete or
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        self.current_signal_state = np.zeros((self.rows * self.cols,1))

        self.change_signal_time = np.zeros((self.rows * self.cols,1))

        if self.tl_type != "actuated":
            for i in range(self.rows * self.cols):
                self.k.traffic_light.set_state(
                    node_id='center' + str(i), state="rrrrrrrrrrrr") # imp do necessary changes
    ...
    def action_space(self):
        ...
        if self.discrete:
            ...
            return Box(
                low=-1,
                high=1,
                shape=(self.num_traffic_lights*12,),
                dtype=np.float32)
    ...
    def observation_space(self):
        ...
        traffic_lights = Box(
            low=0.,
            high=1,
            shape=(2 * self.rows * self.cols * 12,),
            dtype=np.float32)
    ...
    def get_state(self):
        ...
        state = [
            ...
            self.current_signal_state.flatten().tolist(),
            self.change_signal_time.flatten().tolist(),
        ]
    ...
    def _apply_rl_actions(self, rl_actions):
        ...
        change_sequence = {0:"r",1:"g",2:'y'}
        for i in range(len(rl_actions)//12):
            start = 12*i
            end = 12*(i+1)
            phase =""
            sig = 0
            for j in range(start,end):
                self.change_signal_time[i][sig] += self.sim_step
                if( rl_actions[j]>0 and (self.change_signal_time[i][sig]> self.min_switch_time) ):
                    self.current_signal_state[i][sig] += 1
                    phase += change_sequence[self.current_signal_state[i][sig] % 3]
                    self.change_signal_time[i][sig]= 0.0
...
class TrafficLightGridPOEnv(TrafficLightGridEnv):
    ...
    * num_observed: number of vehicles nearest each intersection that is
    ...
    def observation_space(self):
        ...
        state.
        """
        sh = 3 * 4 * self.num_observed * self.num_traffic_lights + 2 * len(self.k.network.get_edge_list()) + self.rows * self.cols * (2)
        # print("sh",sh)
        tl_box = Box(
            low=0.0,
            high=300.0,
    ...
    def action_space(self):
        ...
        if self.discrete:
            ...
            return Box(
                low=-1,
                high=1,
                shape=(self.num_traffic_lights*5,),
                dtype=np.float32)
    ...
    def _apply_rl_actions(self, rl_actions):
        """See class definition."""
        # print("*************************************DD")
        phases = {4: ("ggr"+"rrr"+"ggr"+"rrr"), 1: ("rrr"+"ggr"+"rrr"+"ggr"), 2: ("rrg"+"grr"+"rrg"+"grr"),
                  3: ("grr"+"rrg"+"grr"+"rrg"), 0: ("rrr"+"rrr"+"rrr"+"rrr")}

        for i in range(len(rl_actions)//5):
            start = 5*i
            end = 5*(i+1)
            phase =""
            index = np.argmax(np.array(rl_actions[start:end]))
            self.change_signal_time[i][0] += self.sim_step
            if( self.current_signal_state[i][0] != index and (self.change_signal_time[i][0]> self.min_switch_time) ):
                self.current_signal_state[i][0] = index
                self.change_signal_time[i][0] = 0

            phase = phases.get(index,0)             
